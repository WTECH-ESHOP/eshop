...
function getQueryItem(filterName) {
  ...
  let queryArray = Array.prototype.slice
    .call(checkBoxes)
...
function filterResults() {
  ...
  filterNames.forEach((filterName) => {
    ...
    let queryParam = document
      .getElementsByName(filterName)[0]
      .getAttribute('param')
  ...
  for (let elem of params) {
    elem.addEventListener('click', () => {
      ...
      if (!!urlParams.toString())
        document.location.href = `${baseUrl}?` + urlParams
...
function initializePriceSlider() {
  ...
  handlesSlider.noUiSlider.on('update', function (values) {
    nonLinearStepSliderValueElement.querySelector(
      '#price-min'
    ).textContent = `${(+values[0]).toFixed(0)} €`
    nonLinearStepSliderValueElement.querySelector(
      '#price-max'
    ).textContent = `${(+values[1]).toFixed(0)} €`
  })
