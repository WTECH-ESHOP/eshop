<div class="flex items-center gap-2">
  @auth
    ...
    <div class="flex items-center gap-1">
      ...
      <form method="POST" action="{{ route('logout') }}">
        ...
        <button type="submit" class="w-48p h-48p bg-none flex items-center justify-center">
          <span class="material-icons-round font-xl">exit_to_app</span>
        </button>
      </form>
    </div>
  ...
  <a href="{{ route('cart') }}" class="h-48p bg-none flex gap-1 items-center">
    <div class="w-48p h-48p flex items-center justify-center relative">
      <div class="absolute top-1 left-7 bg-grey rounded-full w-16p h-16p flex items-center justify-center">
        <span class="text-9p font-semibold">{{ $count }}</span>
      </div>
